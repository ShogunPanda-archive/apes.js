...
const HTTPServer = {
  ...
  async create(application, main, prepare, httpConfigurationRoot = "httpServer"){
    ...
    try{
      ...
      console.error(`Cannot load HTTP Server configuration: ${e}. Exiting ...`);
  ...
  /**
   * Set server port and SSL, install a profiler middleware and finally set it up to only accept JSON bodies and to parse them.
   *
   * @param {object} server A server object (created via {@link HTTPServer.create}).
   * @private
   */
  _configureExpress(server){
    ...
    server.ssl = HTTPServer._loadSSL(server);

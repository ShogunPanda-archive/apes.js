...
const HTTPServer = {
  ...
  async execute(configurationRoot, main, prepare = null, root = null){
    if(Utils.isEmptyString(configurationRoot))
      configurationRoot = "apes.httpServer";

    let [key, httpConfigurationRoot] = configurationRoot.split(".", 2); // eslint-disable-line prefer-const

    if(Utils.isEmptyString(httpConfigurationRoot))
      httpConfigurationRoot = "httpServer";

    const application = Application.create(key, root);
    const server = await HTTPServer.create(application, main, prepare, httpConfigurationRoot);
    return await HTTPServer.run(server);
  },
  ...
  async create(application, main, prepare, httpConfigurationRoot = "httpServer"){
    const rv = {
      ...
      configuration: HTTPServer._loadConfiguration(application, httpConfigurationRoot),
      express: express(),
    ...
    rv.port = parseInt(process.env.PORT || rv.configuration.port, 0);
    if(rv.port < 0 || isNaN(rv.port))
      rv.port = HTTPServer.defaultPort;
    rv.ssl = HTTPServer._loadSSL(rv);

    // Add the profiler
    rv.express.use((req, res, next) => HTTPServer.profiler(rv, req, res, next));
    rv.express.enable("trust proxy");

    // Add GZIP compression
    if(rv.application.production)
      rv.express.use(compression({threshold: 0}));

    // Configure Express behavior for JSON handling
    HTTPServer._configureJSONHandling(rv);
   ...
   */
  async run(server){
    return await Application.run(server.application, async () => new Promise(
      (resolve, reject) => {
  ...
  /**
   * Load SSL certificate and key for a HTTPS server.
   *
   * @param {object} server A server object (created via {@link HTTPServer.create}).
   ...
   */
  _loadSSL(server){
    if(server.application.production || (!Utils.parseBoolean(process.env.SSL) && !server.configuration.ssl.enabled)) // eslint-disable-line no-extra-parens
      return false;

    return {
      key: fs.readFileSync(path.resolve(server.application.root, server.configuration.ssl.key || "config/ssl/private-key.pem")), // eslint-disable-line no-sync
      cert: fs.readFileSync(path.resolve(server.application.root, server.configuration.ssl.certificate || "config/ssl/certificate.pem")) // eslint-disable-line no-sync
    };
  },

  /**
   * Set a server to only accept JSON bodies and to parse it.
   *
   * @param {object} server A server object (created via {@link HTTPServer.create}).
   * @private
   */
  _configureJSONHandling(server){

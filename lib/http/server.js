...
(function (HTTP) {
    ...
    function Profiler(req, res, next) {
        ...
        onHeaders(res, () => {
            ...
            res.set("X-Response-Time", sprintf_js_1.sprintf("%0.3fms", utils_1.Utils.elapsedTime(startTime)));
        });
        next();
    }
    HTTP.Profiler = Profiler;
    /**
    ...
    class Server extends application_1.Application {
         ...
         */
        prepare() {
            this.requestsLogger = new logger_1.Logger(`${application_1.Application.processName}-requests`, !application_1.Application.production);
            this.express = express();
            // Add base middlewares and configure Express
            this.express.use(Profiler);
            // Add GZIP compression and parse JSON body
            if (application_1.Application.production)
            ...
            this.express.set("json spaces", application_1.Application.production ? 0 : 2);
        ...
        execute() {
            return new Promise((resolve, reject) => {
                ...
                const useSSL = (process.env.SSL || this.configuration.httpServer.ssl.enabled) && !application_1.Application.production;
        ...
        setupStaticFolder(dir, root) {
            const final = path.resolve(root || application_1.Application.root, dir);
        ...
        sslConfig() {
            return {
                key: fs.readFileSync(path.resolve(application_1.Application.root, this.configuration.httpServer.ssl.key || "config/ssl/private-key.pem")),
                cert: fs.readFileSync(path.resolve(application_1.Application.root, this.configuration.httpServer.ssl.certificate || "config/ssl/certificate.pem"))
            };
        }
        /**
        ...
        logRequest(code, req, startTime) {
            ...
            return Reflect.get(this.requestsLogger, method).call(this.requestsLogger, Object.assign({
                status: code,
                elapsed: utils_1.Utils.elapsedTime(startTime),
